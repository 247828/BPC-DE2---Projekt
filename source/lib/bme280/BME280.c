...
void handle_button_bme(void) {
    uint8_t current_state = GPIO_read(&PIND, BUTTON2_PIN); // Read the current state of the button

    // Detect button press (falling edge detection)
    if (current_state == 0 && last_button_state_bme == 1) {
        // Save the current pressure as reference pressure when button is pressed
        _delay_ms(100); // Debounce delay
        uart_puts("Height Reset.\r\n");          // Debug message
        
        pressure1 = pressure;  // Save current pressure to pressure1

        // Reset height to 0 (optional)
        height_bme = 0.0;          // Reset current height
    }

    // Save the current state for the next comparison
    last_button_state_bme = current_state;
}
